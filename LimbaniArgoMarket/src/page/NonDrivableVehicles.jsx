            ...
            try {
                if (getAllProduct) {
                    ...
                    if (data && Array.isArray(data)) {
                        ...
                        const nonDrivableOnly = normalized.filter(item => isItemNonDrivable(item));
                        setProducts(nonDrivableOnly);
                    }
                }
            } catch (err) {
                console.error("NonDrivableVehicles fetch error:", err);
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <main className="w-full pt-[76px] pb-20 bg-surface dark:bg-surface-dim min-h-screen">
            ...
            <div className="bg-surface-container/50 border-b border-outline-variant/20 py-2.5 mb-6">
                <div className="max-w-container-max mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
                    <nav className="flex items-center gap-1.5 text-xs sm:text-sm text-on-surface-variant font-medium">
                        ...
                        <span className="material-symbols-outlined text-[14px]">chevron_right</span>
                        <span className="text-secondary font-bold">Non - Drivable Vehicles</span>
                    </nav>

                    <Link
                        to="/equipments"
                        className="inline-flex items-center gap-1 px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg sm:rounded-xl bg-secondary/10 hover:bg-secondary text-secondary hover:text-on-secondary border border-secondary/20 hover:border-secondary font-bold text-[10px] sm:text-xs transition-all duration-200 shadow-sm active:scale-95 shrink-0"
                    >
            ...
            <div className="max-w-container-max mx-auto px-4 sm:px-margin-mobile md:px-margin-desktop mb-8">
                <div className="bg-gradient-to-r from-secondary/90 via-secondary to-secondary/80 rounded-3xl p-6 sm:p-8 text-on-secondary card-shadow relative overflow-hidden">
                    <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div>
                            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/20 text-white font-bold text-xs mb-3 backdrop-blur-md border border-white/30">
                                <span className="material-symbols-outlined text-sm">handyman</span> Rotavators, Cultivators & Implements
                            </div>
                            <h1 className="font-display-lg text-2xl sm:text-4xl font-extrabold text-white mb-2">
                                Non - Drivable Vehicles & Implements
                            </h1>
                            <p className="text-white/90 text-xs sm:text-sm max-w-xl font-medium">
                                Browse all available rotavators, seed drills, plows, threshers, sprayers, and farming attachments across India.
                            </p>
                        </div>
                        ...
                        <div className="bg-white/10 backdrop-blur-md border border-white/20 px-5 py-3 rounded-2xl text-center shrink-0">
                            ...
                            <span className="text-[11px] sm:text-xs font-bold text-white/80 uppercase tracking-wider">Total Implements</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-container-max mx-auto px-4 sm:px-margin-mobile md:px-margin-desktop">
                ...
                <div className="mb-6 flex flex-col sm:flex-row items-center justify-between gap-4 bg-surface-container-lowest p-4 rounded-2xl border border-outline-variant/30 card-shadow">
                    <div className="relative w-full sm:max-w-md">
                        ...
                        <input
                            type="text"
                            placeholder="Search rotavator, cultivator, seed drill..."
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full pl-10 pr-4 py-2.5 bg-surface-container/50 border border-outline-variant/30 rounded-xl text-xs sm:text-sm font-medium text-on-surface focus:outline-none focus:border-secondary transition-all"
                        />
                        {searchTerm && (
                            <button
                    ...
                    <div className="text-xs font-bold text-on-surface-variant self-end sm:self-center">
                        Showing {filtered.length} of {products.length} Non-Drivable Vehicles
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="text-center py-16 bg-surface-container-lowest rounded-3xl border border-outline-variant/30 card-shadow">
                        <span className="material-symbols-outlined text-5xl text-outline mb-2">construction</span>
                        <p className="text-on-surface font-bold text-base mb-1">No Non-Drivable Vehicles Found</p>
                        <p className="text-on-surface-variant text-xs mb-4">Try searching with a different keyword or view all listings.</p>
                        {searchTerm && (
                            <button
                                onClick={() => setSearchTerm('')}
                                className="px-4 py-2 bg-secondary text-on-secondary font-bold text-xs rounded-xl hover:bg-secondary/90 transition-all"
                            >
                                Reset Search
                            </button>
                        )}
...
export default NonDrivableVehicles;
